  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="space-y-4">
            {[1, 2].map(i => (
              <div key={i} className="card animate-pulse flex gap-4">
                <div className="w-24 h-16 bg-slate-800 rounded-lg shrink-0" />
              ...
              return (
                <div key={e.id} className="card flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                  {e.course?.thumbnail_url ? (
                    ...
                    <div className="w-full sm:w-28 h-20 bg-gradient-to-br from-blue-950 to-slate-900 rounded-lg shrink-0 flex items-center justify-center">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-white font-semibold mb-1 truncate">{e.course?.title}</h3>
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-500">
                      <span>{e.progress?.completed_lessons || 0}/{e.progress?.total_lessons || 0} lessons</span>
                    </div>
                  </div>
                  <div className="flex gap-2 shrink-0">
                    {done && (
                      <Link to={`/certificate/${e.id}`}
                        className="text-xs bg-green-950 border border-green-900 text-green-400 px-3 py-1.5 rounded-lg hover:bg-green-900 transition-colors">
                        Certificate
                      </Link>
                    )}
        ...
        <div className="mt-8">
          <Link to="/courses" className="text-blue-400 hover:text-blue-300 text-sm transition-colors">
            ← Browse more courses
          </Link>
        </div>
      </div>
