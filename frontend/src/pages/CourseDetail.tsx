  ...
  return (
    <div className="min-h-screen">
          ...
          <div className="flex flex-col lg:flex-row gap-10">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-4">
                <Link to="/courses" className="text-slate-400 hover:text-white text-sm transition-colors">
                  Courses
                </Link>
              ...
              <p className="text-slate-400 leading-relaxed mb-6">{course.short_description}</p>
              ...
              <div className="card sticky top-24">
                ...
                <div className="flex items-baseline gap-2 mb-4">
                  ...
                  {course.original_price && (
                    <span className="text-slate-500 line-through">₹{course.original_price}</span>
                  )}
                  {course.original_price && (
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <p className="text-slate-400 text-sm mb-8">{course.description}</p>

        <div className="space-y-3">
          {course.modules?.map(module => (
            <div key={module.id} className="border border-slate-800 rounded-xl overflow-hidden">
              <button
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <span className="text-slate-400 text-sm">{module.lessons?.length || 0} lessons</span>
                  <svg
                    className={`w-4 h-4 text-slate-400 transition-transform ${openModule === module.id ? 'rotate-180' : ''}`}
              ...
              {openModule === module.id && (
                <div className="divide-y divide-slate-800">
                  {module.lessons?.map(lesson => (
                    <div key={lesson.id} className="flex items-center gap-3 px-5 py-3 bg-slate-950">
                      ...
                      <span className="text-slate-500 text-xs">{fmtDuration(lesson.duration_seconds)}</span>
                    </div>
                  ))}
                </div>
              )}
